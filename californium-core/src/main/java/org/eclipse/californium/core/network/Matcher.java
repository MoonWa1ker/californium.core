...
public class Matcher {
	...
	//andrianeshsg: In case the request was marked as multicast
	//we need to support the reception of multiple responses.
	public Exchange receiveResponse(Response response) {
		...
		if (exchange != null) {
			Boolean multR = exchange.getRequest().isMulticast();
			// There is an exchange with the given token
			Exchange prev = null;
			if(!multR)
				prev = deduplicator.findPrevious(idByMID, exchange);
			else
				prev = deduplicator.find(idByMID);
	...
	private class ExchangeObserverImpl implements ExchangeObserver {
		...
		public void completed(Exchange exchange) {
			...
			if (exchange.getOrigin() == Origin.LOCAL) {
				...
				LOGGER.info("[CLIENT]EXCHANGE COMPLETED");
...
//				LOGGER.fine("Exchange completed: Cleaning up "+idByTok);
				...
				LOGGER.info("[SERVER]EXCHANGE COMPLETED");
